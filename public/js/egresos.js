...
function renderTabla(rows) {
  ...
  tbody.innerHTML = rows.map(e => `
    <tr>
      ...
      <td>${(() => { const s = semaforoVto(e.fecha_vto_pago, e.estado); return `<span title="${s.titulo}" style="color:${s.color}; font-size:14px">●</span>`; })()} ${formatFecha(e.fecha_vto_pago)}</td>
...
// Semáforo de vencimiento (sobre la fecha de vto. de pago):
//   verde  = en fecha (no vencido)
