...
// Devuelve los datos del medio de pago según la opción elegida
function datosMedioPago() {
  const medio = document.getElementById('p-medio_pago').value;
  const val = id => document.getElementById(id)?.value || null;
  if (medio === 'Transferencia') {
    return {
      transf_banco: val('p-transf_banco'),
      transf_cuenta: val('p-transf_cuenta'),
      transf_titular: val('p-transf_titular'),
    };
  }
  if (medio === 'Cheque') {
    return {
      cheque_banco: val('p-cheque_banco'),
      cheque_numero: val('p-cheque_numero'),
      cheque_titular: val('p-cheque_titular'),
      cheque_fecha_emision: val('p-cheque_fecha_emision'),
      cheque_fecha_cobro: val('p-cheque_fecha_cobro'),
    };
  }
  return {};
}

// Muestra/oculta los campos según el medio de pago elegido
function toggleMedioPago() {
  const medio = document.getElementById('p-medio_pago').value;
  document.getElementById('datos-transferencia').style.display = medio === 'Transferencia' ? 'block' : 'none';
  document.getElementById('datos-cheque').style.display = medio === 'Cheque' ? 'block' : 'none';
}
document.getElementById('p-medio_pago').addEventListener('change', toggleMedioPago);
toggleMedioPago();
...
async function confirmarCorrida(tipo) {
  ...
  try {
    ...
    const corrida = await apiFetch('/api/corridas', {
      method: 'POST',
      body: { egreso_ids, tipo, fecha_pago, medio_pago, ...datosMedioPago() },
    });
