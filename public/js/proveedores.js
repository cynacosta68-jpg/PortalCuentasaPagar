...
function abrirModal(proveedor = null) {
  ...
  if (proveedor) {
    ...
    $('#f-cert_no_retencion').checked = !!proveedor.cert_no_retencion;
    if (proveedor.cert_no_ret_desde) $('#f-cert_no_ret_desde').value = String(proveedor.cert_no_ret_desde).slice(0, 10);
    if (proveedor.cert_no_ret_hasta) $('#f-cert_no_ret_hasta').value = String(proveedor.cert_no_ret_hasta).slice(0, 10);
    toggleCertNoRet();
  } else {
...
$('#f-cert_no_retencion').addEventListener('change', toggleCertNoRet);

function toggleCertNoRet() {
  $('#cert-vigencia').style.display = $('#f-cert_no_retencion').checked ? 'grid' : 'none';
}
...
$('#btn-guardar').addEventListener('click', async () => {
  ...
  const payload = {
    ...
    cert_no_retencion: $('#f-cert_no_retencion').checked,
    cert_no_ret_desde: $('#f-cert_no_retencion').checked ? ($('#f-cert_no_ret_desde').value || null) : null,
    cert_no_ret_hasta: $('#f-cert_no_retencion').checked ? ($('#f-cert_no_ret_hasta').value || null) : null,
  };
...
function limpiarForm() {
  ...
  $('#f-cert_no_retencion').checked = false;
  $('#f-cert_no_ret_desde').value = '';
  $('#f-cert_no_ret_hasta').value = '';
  $('#cert-vigencia').style.display = 'none';
