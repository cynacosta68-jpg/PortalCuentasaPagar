...
async function crear(req, res) {
  ...
  const { rows } = await pool.query(
    `INSERT INTO proveedores (
       ...
       cert_no_retencion, cert_no_ret_desde, cert_no_ret_hasta,
    ...
    [
      ...
      b.cert_no_retencion || false, b.cert_no_ret_desde || null, b.cert_no_ret_hasta || null,
...
async function actualizar(req, res) {
  ...
  const { rows } = await pool.query(
    `UPDATE proveedores SET
       ...
       cert_no_retencion = COALESCE($18, cert_no_retencion),
       cert_no_ret_desde = $19,
       cert_no_ret_hasta = $20,
       updated_at = now()
     WHERE id = $17
     RETURNING *`,
    [
      ...
      req.params.id,
      b.cert_no_retencion ?? null,
      b.cert_no_retencion ? (b.cert_no_ret_desde || null) : null,
      b.cert_no_retencion ? (b.cert_no_ret_hasta || null) : null,
    ]
  );
  if (!rows.length) return res.status(404).json({ error: 'Proveedor no encontrado' });
