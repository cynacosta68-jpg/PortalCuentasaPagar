...
async function crear(req, res) {
  ...
  const { rows } = await pool.query(
    `INSERT INTO egresos (
       ...
       moneda, cotizacion,
       concepto, categoria_egreso, fecha_vto_pago, origen,
    ...
    [
      ...
      b.concepto || null, b.categoria_egreso || null, b.fecha_vto_pago || null,
      b.origen || 'manual',
...
async function actualizar(req, res) {
  ...
  const { rows } = await pool.query(
    `UPDATE egresos SET
     ...
     RETURNING *`,
    [b.concepto, b.categoria_egreso, b.fecha_vto_pago, req.params.id]
  );
  if (!rows.length) return res.status(404).json({ error: 'Egreso no encontrado o no editable' });
  res.json(rows[0]);
}
