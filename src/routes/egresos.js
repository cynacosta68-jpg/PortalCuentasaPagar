...
const { leerMultipart, parsearMultipart } = require('../services/multipart');

function register(router) {
  router.get('/api/egresos', listar);
  router.get('/api/egresos/:id', obtener);
  router.post('/api/egresos/:id/comprobante', subirComprobante);
  router.get('/api/egresos/:id/comprobante', verComprobante);
...
async function listar(req, res) {
  ...
  const { rows } = await pool.query(
    `SELECT e.id, e.tipo_comprobante, e.punto_venta, e.numero,
            ...
            (e.comprobante_pdf IS NOT NULL) AS tiene_comprobante,
...
async function obtener(req, res) {
  ...
  delete rows[0].comprobante_pdf; // no enviar el binario en el JSON
  res.json(rows[0]);
}

// Guarda el PDF del comprobante asociado al egreso (multipart)
async function subirComprobante(req, res) {
  const contentType = req.headers['content-type'] || '';
  const boundaryMatch = contentType.match(/boundary=([^\s;]+)/);
  if (!boundaryMatch) return res.status(400).json({ error: 'Se esperaba multipart/form-data' });
  ...
  if (!archivo) return res.status(400).json({ error: 'No se encontró un archivo PDF' });
  if (archivo.data.length > 10 * 1024 * 1024) {
    return res.status(400).json({ error: 'El PDF no puede superar 10 MB' });
  }

  const { rows } = await pool.query(
    `UPDATE egresos SET comprobante_pdf = $1, comprobante_nombre = $2, updated_at = now()
     WHERE id = $3 RETURNING id`,
    [archivo.data, archivo.filename, req.params.id]
  );
  if (!rows.length) return res.status(404).json({ error: 'Egreso no encontrado' });
  res.json({ ok: true, nombre: archivo.filename });
}

// Devuelve el PDF del comprobante para verlo en el navegador
async function verComprobante(req, res) {
  const { rows } = await pool.query(
    'SELECT comprobante_pdf, comprobante_nombre FROM egresos WHERE id = $1',
    [req.params.id]
  );
  if (!rows.length || !rows[0].comprobante_pdf) {
    return res.status(404).json({ error: 'Este egreso no tiene comprobante adjunto' });
  }
  const pdf = rows[0].comprobante_pdf;
  const nombre = (rows[0].comprobante_nombre || `comprobante_${req.params.id}.pdf`).replace(/"/g, '');
  res.writeHead(200, {
