...
// Normaliza una fecha (objeto Date de PG o string) a 'YYYY-MM-DD'
...
function certVigente(proveedor, fechaPago) {
  const f = aFechaISO(fechaPago);
  const desde = aFechaISO(proveedor.cert_no_ret_desde);
  const hasta = aFechaISO(proveedor.cert_no_ret_hasta);
  if (!f) return true;
