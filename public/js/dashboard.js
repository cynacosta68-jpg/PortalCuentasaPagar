...
(async () => {
  ...
  const v = n => V.getPropertyValue('--' + n).trim() || '#7c3aed';
  const violetas = [v('v1'), v('v2'), v('v3'), v('v4'), v('v5'), v('v6')];
  ...
  try {
    ...
    // ── 1. Egresos devengados (barras finas, violeta) ───
    new Chart(document.getElementById('chart-tendencia'), {
      ...
      options: {
        ...
        scales: {
          ...
          y: { grid: { display: false }, ticks: { font: { size: 11 }, callback: x => '$ ' + (x / 1000).toLocaleString('es-AR') + 'k' } },
        },
      },
    });
    ...
    // ── 3. Salida de fondos (línea, violeta) ────────────
    new Chart(document.getElementById('chart-pagos'), {
      ...
      data: {
        ...
        datasets: [{
          ...
          borderColor: v('v1'), backgroundColor: 'rgba(109,40,217,.10)',
    ...
    new Chart(document.getElementById('chart-proveedores'), {
      ...
      options: {
        ...
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => ` ${formatImporte(ctx.raw)}` } } },
        scales: {
          x: { grid: { display: false }, ticks: { font: { size: 11 }, callback: x => '$ ' + (x / 1000).toLocaleString('es-AR') + 'k' } },
...
function renderConsultas(lista) {
  ...
  const estiloEstado = {
    ...
    cerrado:    { bg: '#f1ecfd', tx: '#6d28d9', t: 'Cerrado' },
  };
