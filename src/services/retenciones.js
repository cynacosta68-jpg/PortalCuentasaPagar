...
async function calcularRetencionesOrden(proveedor, egresos, opciones = {}) {
  ...
  // Certificado de NO retención / exclusión: si está vigente a la fecha de pago,
  // no se practica NINGUNA retención mientras dure esa condición.
  let fechaPago = opciones.fechaPago
    ? String(opciones.fechaPago).slice(0, 10)
    : (opciones.periodo ? `${opciones.periodo}-15` : new Date().toISOString().slice(0, 10));
  if (proveedor.cert_no_retencion && certVigente(proveedor, fechaPago)) {
    return {
      importeBruto: redondear(importeBruto),
      importeNeto: redondear(importeNeto),
      importeIva: redondear(importeIva),
      retGanancias: { ...vacio }, retencionesGanancias: [],
      retIva: { ...vacio }, retIibb: { ...vacio }, retSuss: { ...vacio },
      totalRetenciones: 0,
      importeNetoPago: redondear(importeBruto),
      certNoRetencion: true,
    };
  }
...
// ¿El certificado de no retención del proveedor está vigente a la fecha de pago?
function certVigente(proveedor, fechaPago) {
  const f = fechaPago; // 'YYYY-MM-DD'
  const desde = proveedor.cert_no_ret_desde ? String(proveedor.cert_no_ret_desde).slice(0, 10) : null;
  const hasta = proveedor.cert_no_ret_hasta ? String(proveedor.cert_no_ret_hasta).slice(0, 10) : null;
