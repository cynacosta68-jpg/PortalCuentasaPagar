...
function register(router) {
  ...
  router.get('/api/dashboard/demora-pago', demoraPago);
...
async function demoraPago(req, res) {
  // Clasifica los egresos pagados según cuándo se pagaron respecto del vto. de pago
  const { rows: [r] } = await pool.query(
    `SELECT
       COUNT(*) FILTER (WHERE c.fecha_pago <  e.fecha_vto_pago) AS antes_vto,
       COUNT(*) FILTER (WHERE c.fecha_pago =  e.fecha_vto_pago) AS en_vto,
       COUNT(*) FILTER (WHERE c.fecha_pago >  e.fecha_vto_pago
                          AND (c.fecha_pago - e.fecha_vto_pago) <= 30) AS venc_30,
       COUNT(*) FILTER (WHERE (c.fecha_pago - e.fecha_vto_pago) > 30) AS venc_mas30
     FROM egresos e
     JOIN corridas_pago c ON c.id = e.corrida_pago_id
     WHERE e.estado = 'pagado'
       AND e.fecha_vto_pago IS NOT NULL
       AND c.fecha_pago IS NOT NULL`
  );
  res.json({
    antes_vto:  parseInt(r.antes_vto),
    en_vto:     parseInt(r.en_vto),
    venc_30:    parseInt(r.venc_30),
    venc_mas30: parseInt(r.venc_mas30),
  });
}
