...
function extraerCategoriaMonotributo(root) {
  ...
  if (!dm) return null;
  const cat = dm.categoriaMonotributo || dm.descripcionCategoria || deepFind(dm, 'categoriaMonotributo');
  if (cat && typeof cat === 'object') return cat.descripcionCategoria || cat.categoria || null;
  return cat || null;
}
