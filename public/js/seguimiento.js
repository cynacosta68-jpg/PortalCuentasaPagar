...
async function verDetalle(id) {
  ...
  if (corrida.estado === 'aprobada') {
    acciones = `<button class="btn btn-primary" data-cap="ejecutar" onclick="ejecutarCorrida(${id})">⚡ Ejecutar pago</button>`;
  }
  if (corrida.estado === 'pendiente_aprob' || corrida.estado === 'borrador') {
    acciones = `<button class="btn btn-primary" data-cap="corridas" onclick="reenviarAprobacion(${id})">↻ Reenviar pedido de aprobación</button>`;
  }
  document.getElementById('det-acciones').innerHTML = acciones;
  if (window.aplicarPermisos) window.aplicarPermisos(document.getElementById('det-acciones'));

  // Medio de pago + datos de la cuenta/cheque
  document.getElementById('det-medio').innerHTML = renderMedioPago(corrida);
...
// Muestra el medio de pago de la corrida y sus datos (transferencia / cheque)
function renderMedioPago(c) {
  if (!c.medio_pago) return '';
  const fecha = f => f ? formatFecha(f) : '—';
  let detalle = '';
  if (c.medio_pago === 'Transferencia' && (c.transf_banco || c.transf_cuenta || c.transf_titular)) {
    detalle = `Banco: <strong>${c.transf_banco || '—'}</strong> · Cuenta/CBU: <strong>${c.transf_cuenta || '—'}</strong> · Titular: <strong>${c.transf_titular || '—'}</strong>`;
  } else if (c.medio_pago === 'Cheque' && (c.cheque_banco || c.cheque_numero)) {
    detalle = `Banco: <strong>${c.cheque_banco || '—'}</strong> · Nro: <strong>${c.cheque_numero || '—'}</strong> · Titular: <strong>${c.cheque_titular || '—'}</strong> · Emisión: <strong>${fecha(c.cheque_fecha_emision)}</strong> · Cobro: <strong>${fecha(c.cheque_fecha_cobro)}</strong>`;
  }
  return `
    <div style="background:var(--accent-light); border:1px solid var(--border); border-radius:var(--radius); padding:12px 14px; font-size:13px">
      <span style="text-transform:uppercase; font-size:11px; letter-spacing:.5px; color:var(--text-muted)">Medio de pago</span><br>
      <strong>${c.medio_pago}</strong>${detalle ? `<div style="margin-top:4px; color:var(--text-muted)">${detalle}</div>` : ''}
    </div>`;
}
