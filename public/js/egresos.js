...
let pdfComprobante = null; // archivo PDF a guardar junto al egreso
  ...
  tbody.innerHTML = rows.map(e => `
    <tr>
      ...
      <td>
        ...
        ${e.estado === 'pendiente' ? `<button class="btn btn-secondary btn-sm" data-cap="cargar" onclick="editarEgreso(${e.id})">Editar</button>` : ''}
        ${e.estado === 'pendiente' ? `<button class="btn btn-danger btn-sm" data-cap="cargar" onclick="anularEgreso(${e.id})">Eliminar</button>` : ''}
      </td>
    </tr>`).join('');
  if (window.aplicarPermisos) window.aplicarPermisos(tbody);
}
...
async function procesarPdf(file) {
  ...
  try {
    ...
    pdfComprobante = file; // guardar el PDF para adjuntarlo al egreso
    alertEl.innerHTML = '';
...
// Adjuntar PDF opcional del comprobante (carga manual o QR)
document.getElementById('f-comprobante-adjunto')?.addEventListener('change', e => {
  const file = e.target.files[0];
  const info = document.getElementById('adjunto-info');
  if (file && file.type === 'application/pdf') {
    pdfComprobante = file;
    if (info) info.textContent = `✓ ${file.name} — se guardará con el egreso`;
  } else {
    pdfComprobante = null;
    if (info) info.textContent = file ? 'El archivo debe ser un PDF' : '';
  }
});
...
async function guardarEgreso() {
  ...
  try {
    if (egresoEditId) {
      ...
      if (pdfComprobante) await subirComprobante(egresoEditId, pdfComprobante);
      cerrarModal();
      showAlert('Egreso actualizado correctamente', 'success');
    } else {
      const nuevo = await apiFetch('/api/egresos', { method: 'POST', body: payload });
      if (pdfComprobante && nuevo?.id) await subirComprobante(nuevo.id, pdfComprobante);
      cerrarModal();
...
// Sube el PDF del comprobante asociado a un egreso
async function subirComprobante(egresoId, file) {
  const fd = new FormData();
  fd.append('archivo', file, file.name);
  const res = await fetch(`/api/egresos/${egresoId}/comprobante`, { method: 'POST', body: fd });
  if (!res.ok) {
    const d = await res.json().catch(() => ({}));
    throw new Error(d.error || 'No se pudo guardar el comprobante PDF');
  }
}
...
async function verDetalle(id) {
  ...
  document.getElementById('detalle-body').innerHTML = `
    ...
    ${e.tiene_comprobante
      ? `<div style="margin-top:14px"><a class="btn btn-primary" href="/api/egresos/${e.id}/comprobante" target="_blank" rel="noopener">📄 Ver comprobante</a></div>`
      : `<div style="margin-top:14px; font-size:13px; color:var(--text-muted)">Sin comprobante PDF adjunto.</div>`}
  `;
...
async function anularEgreso(id) {
  if (!confirm('¿Eliminar este egreso? Se anulará y saldrá del listado de pendientes. Esta acción no se puede deshacer.')) return;
  try {
...
function limpiarModal() {
  ...
  pdfComprobante = null;
  const adj = document.getElementById('f-comprobante-adjunto');
  if (adj) adj.value = '';
  const info = document.getElementById('adjunto-info');
